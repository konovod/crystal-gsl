    ...
    begin
      ...
      max_iter.times do |i|
        unless LibGSL::Code.new(LibGSL.gsl_multiroot_fsolver_iterate(raw)).gsl_success?
          return GSL::Result::NoConvergence, GSL::Vector.new(raw.value.x).clone
        end
        ...
        if dx_passed || f_passed
          return GSL::Result::Success, GSL::Vector.new(raw.value.x).clone
        end
      end
      return GSL::Result::IterationLimit, GSL::Vector.new(raw.value.x).clone
    ensure
