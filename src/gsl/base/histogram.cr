...
module GSL
  abstract class AbstractHistogram
    ...
    def bin(n : Int) : Float64
      return LibGSL.histogram_get(@histogram, n)
    end

    def range(n : Int) : Tuple(Float64, Float64)
      min = 0.0
      max = 0.0
      LibGSL.histogram_get_range(@histogram, n, pointerof(min), pointerof(max))
      return {min, max}
    end

    def find(x : Float64) : UInt64
      index = 0.to_u64
      LibGSL.histogram_find(@histogram, x, pointerof(index))
      return index
    end

    # This function returns the maximum value contained in the histogram bins.
    def max_val : Float64
      return LibGSL.histogram_max_val @histogram
    end

    # This function returns the minimum value contained in the histogram bins.
    def min_val : Float64
      return LibGSL.histogram_min_val @histogram
    end
    ...
    def max_bin : UInt64
      return LibGSL.histogram_max_bin @histogram
    end

    # This function returns the index of the bin containing the minimum value.
    # In the case where several bins contain the same maximum value the smallest index is returned.
    def min_bin : UInt64
      return LibGSL.histogram_min_bin @histogram
    end
    ...
    def mean : Float64
      return LibGSL.histogram_mean @histogram
    end
    ...
    def equal_bins(h : AbstractHistogram) : Bool
      return LibGSL.histogram_equal_bins_p(@histogram, h.getHistogram) == 1 ? true : false
    end

    # Returns an exact copy of this histogram
    def clone : Histogram
      return Histogram.new LibGSL.histogram_clone @histogram
    end
  end

  class Histogram < AbstractHistogram
    def initialize(data : Array(Float), bins : Array(Float))
      @histogram = LibGSL.histogram_alloc bins.size - 1
      LibGSL.histogram_set_ranges(@histogram, bins, bins.size)
      data.each { |x| LibGSL.histogram_increment @histogram, x }
    end

    protected def initialize(@histogram : LibGSL::Histogram*)
    end
    ...
    def initialize(data : Array(Float), bins : Int32)
      ...
      @histogram = LibGSL.histogram_alloc bins
      LibGSL.histogram_set_ranges_uniform @histogram, data.min, data.max
      data.each { |x| LibGSL.histogram_increment @histogram, x }
    end
    ...
    def +(h : Histogram | MutableHistogram) : Histogram
      ...
      dest_hist = LibGSL.histogram_clone @histogram
      LibGSL.histogram_add(dest_hist, h.getHistogram)
    ...
    def -(h : Histogram | MutableHistogram) : Histogram
      ...
      dest_hist = LibGSL.histogram_clone @histogram
      LibGSL.histogram_sub(dest_hist, h.getHistogram)
      return Histogram.new dest_hist
    end
    ...
    def *(h : Histogram | MutableHistogram) : Histogram
      ...
      dest_hist = LibGSL.histogram_clone @histogram
      LibGSL.histogram_mul(dest_hist, h.getHistogram)
      return Histogram.new dest_hist
    end
    ...
    def /(h : Histogram | MutableHistogram) : Histogram
      ...
      dest_hist = LibGSL.histogram_clone @histogram
      LibGSL.histogram_div(dest_hist, h.getHistogram)
      return Histogram.new dest_hist
    end
    ...
    def *(scale : Float64) : Histogram
      dest_hist = LibGSL.histogram_clone @histogram
      LibGSL.histogram_scale(dest_hist, scale)
      return Histogram.new dest_hist
    end
    ...
    def +(offset : Float64) : Histogram
      dest_hist = LibGSL.histogram_clone @histogram
      LibGSL.histogram_shift(dest_hist, offset)
      return Histogram.new dest_hist
    end
  end
  ...
  class MutableHistogram < AbstractHistogram
    ...
    def initialize(min : Float64, max : Float64, bins : Int32)
      @histogram = LibGSL.histogram_alloc bins
      LibGSL.histogram_set_ranges_uniform @histogram, min, max
    end
    ...
    def increment(x : Float64)
      LibGSL.histogram_increment(@histogram, x)
    end

    def accumulate(x : Float64, weight : Float64)
      LibGSL.histogram_accumulate(@histogram, x, weight)
    end
    ...
    def +(h : Histogram | MutableHistogram) : MutableHistogram
      ...
      LibGSL.histogram_add(@histogram, h.getHistogram)
      return self
    end
    ...
    def -(h : Histogram | MutableHistogram) : MutableHistogram
      ...
      LibGSL.histogram_sub(@histogram, h.getHistogram)
      return self
    end
    ...
    def *(h : Histogram | MutableHistogram) : MutableHistogram
      ...
      LibGSL.histogram_mul(@histogram, h.getHistogram)
      return self
    end
    ...
    def /(h : Histogram | MutableHistogram) : MutableHistogram
      ...
      LibGSL.histogram_div(@histogram, h.getHistogram)
      return self
    end
    ...
    def *(scale : Float64) : MutableHistogram
      LibGSL.histogram_scale(@histogram, scale)
      return self
    end
    ...
    def +(offset : Float64) : MutableHistogram
      LibGSL.histogram_shift(@histogram, offset)
      return self
    end
  end
end
