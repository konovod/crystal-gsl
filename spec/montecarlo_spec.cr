...
describe GSL::MonteCarlo do
  describe "Plain" do
    it "should give correct result" do
      ...
      x, err = GSL::MonteCarlo.integrate_plain(f, lower, upper, 10000)
      err.should be < 1.0
      x.should be_close exact, 0.5
    end
  end
  describe "Miser" do
    it "should give correct result" do
      a = 1.0 / (Math::PI * Math::PI * Math::PI)
      f = ->(k : Slice(Float64)) { a / (1.0 - Math.cos(k[0]) * Math.cos(k[1]) * Math.cos(k[2])) }
      x, err = GSL::MonteCarlo.integrate_miser(f, lower, upper, 10000)
      err.should be < 1.0
      x.should be_close exact, 0.5
    end
    ...
    it "can be configured" do
      a = 1.0 / (Math::PI * Math::PI * Math::PI)
      f = ->(k : Slice(Float64)) { a / (1.0 - Math.cos(k[0]) * Math.cos(k[1]) * Math.cos(k[2])) }
      x, err = GSL::MonteCarlo.integrate_miser(f, lower, upper, 10000, alpha: 1.5, dither: 0.05, min_calls: 100)
      err.should be < 1.0
      x.should be_close exact, 0.5
    end
  end
  describe "Vegas" do
    it "should give correct result" do
      a = 1.0 / (Math::PI * Math::PI * Math::PI)
      f = ->(k : Slice(Float64)) { a / (1.0 - Math.cos(k[0]) * Math.cos(k[1]) * Math.cos(k[2])) }
      x, err = GSL::MonteCarlo.integrate_vegas(f, lower, upper, 10000)
      err.should be < 0.1
      x.should be_close exact, 0.5
    end
    ...
    it "can be configured" do
      a = 1.0 / (Math::PI * Math::PI * Math::PI)
      f = ->(k : Slice(Float64)) { a / (1.0 - Math.cos(k[0]) * Math.cos(k[1]) * Math.cos(k[2])) }
      x, err = GSL::MonteCarlo.integrate_vegas(f, lower, upper, 10000, alpha: 1.3, iterations: 10, sampling: :stratified)
      err.should be < 0.1
      x.should be_close exact, 0.5
    end
    ...
    it "can be run with control of convergence" do
      ...
      x, err = GSL::MonteCarlo.integrate_vegas(f, lower, upper, alpha: 1.3, iterations: 10, sampling: :stratified) do |v|
        # Warm up
        v.perform(100)
        v.perform(1000, stage: :keep_average)
        v.params(alpha: 1.0)
        loop do
          v.perform(2000, iterations: 5)
